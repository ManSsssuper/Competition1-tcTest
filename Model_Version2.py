...
#得到基础特征
def get_fea_base(op,tr,df_by_uid,tr_fields):
    op_fields=[]
#    tr_fields=[]
...
#得到训练集和测试集顺便补充缺失值-1
not_use=["UID","day","time"]
cols=[]
for field in train_tr.columns:
    if field not in not_use:
        cols.append(field)    
for field in cols:
    tr_fields=[]
    ...
    for f in cols:
        if f !=field:
            tr_fields.append(f)
    train=get_fea_base(train_op,train_tr,train_fts,tr_fields)
    test=get_fea_base(test_op,test_tr,test_fts,tr_fields)
    print(train.shape)
